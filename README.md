...
Атом в `value`, `checked` или `selected` — это связка: regan и пишет в элемент,
и кладёт введённое обратно в атом. Обработчик для этого не нужен:
...
обратную запись отменяют:

```tsx
import {notBind} from 'regan';
...
`notBind` обязателен и для производного атома — в `select` писать нельзя.
