...
`keydown`. Вторым аргументом приходит элемент.
...
```
Invalid child of type object in <App><ul:0><Row:4><li:0>
```

Имя в угловых скобках, через двоеточие — номер среди детей родителя. Номера
верны всегда, а имена берутся у функций компонентов, и минификатор их
переименовывает.
