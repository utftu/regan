...
- **`src/utils/attributes.ts`** — во что превратить значение пропа в разметке:
  ...
  снимает. Плюс `style` объектом и короткий список пропов (`value`, `checked`,
  `selected`), которые на клиенте пишутся свойством элемента вместо атрибута —
  иначе поле ввода перестаёт слушаться после первого ввода.
